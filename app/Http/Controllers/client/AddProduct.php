...
class AddProduct extends Controller
{
    ...
    public function edit_product(Request $r){
        ...
        if(!empty($r->file('image'))){
                $img_data = Products::where([ ['client_users_id', '=', $user_id], ['product_id', '=', $product_id] ])->first();
                $image_path = public_path().'/clientUser/clientUser/'.$img_data->img_banner;
            if(unlink($image_path)){
                $img = time().'.'.$r->file('image')->getClientOriginalName();
                $path = $r->file('image')->store('public/clientUser/clientUser');
                $data=Products::where([ ['client_users_id', '=', $user_id], ['product_id', '=', $product_id] ])
                ->update([
                    'client_users_id'=>$r->user_id,
                    'title'=>$r->title,
                    'about'=>$r->about,
